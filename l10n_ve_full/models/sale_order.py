    ...
    def _compute_ve_sale_dual(self):
        for order in self:
            ...
            if is_bs:
                ...
                order.l10n_ve_untaxed_bs = round(order.amount_untaxed * rate_val, 2)
                order.l10n_ve_tax_bs = round(order.amount_tax * rate_val, 2)
                order.l10n_ve_total_bs = round(order.amount_total * rate_val, 2)

    @api.model
    def default_get(self, default_fields):
        rec = super().default_get(default_fields)
        if 'pricelist_id' in default_fields and self.env.company.l10n_ve_active:
