...
class ProductTemplate(models.Model):
    ...
    l10n_ve_list_price_usd = fields.Float(
        ...
        compute='_compute_ve_list_prices',
    ...
    l10n_ve_standard_price_bs = fields.Monetary(
        string='Costo (Bs)',
        currency_field='l10n_ve_currency_bs_id',
        compute='_compute_ve_standard_prices',
    ...
    @api.depends('list_price', 'company_id', 'company_id.currency_id')
    def _compute_ve_list_prices(self):
    ...
    @api.depends('standard_price', 'company_id', 'company_id.currency_id')
    def _compute_ve_standard_prices(self):
        for template in self:
            company = template.company_id or self.env.company
            rate = company.get_current_bcv_rate() if hasattr(company, 'get_current_bcv_rate') else 60.0
            if not rate or rate <= 0:
    ...
    def _inverse_ve_profit_margin(self):
        for template in self:
            cost_usd = template.standard_price
            if cost_usd > 0:
