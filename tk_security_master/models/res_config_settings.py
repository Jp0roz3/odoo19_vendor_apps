...
class SecurityMasterConfig(models.TransientModel):
    ...
    enable_access_restriction_by_ip = fields.Boolean(string='Is Restriction By IP?',
                                                     config_parameter='tk_security_master.enable_access_restriction_by_ip')
    access_restriction_type = fields.Selection([
        ('range', 'IP Range (e.g., 192.168.1.1 - 192.168.1.100)'),
        ('single', 'Single IP Address (e.g., 192.168.1.1)')
    ], default='single', string='Restriction Type',
        config_parameter='tk_security_master.access_restriction_type')
    unauthorized_notification_enabled = fields.Boolean(
        ...
        config_parameter='tk_security_master.unauthorized_notification_enabled'
    )
