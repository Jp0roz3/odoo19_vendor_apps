...
import { getDualRate, fmt2, getSecondarySymbol, getMainSymbol, getRateLabel, mainToSecondary, secondaryToMain, posInstance } from "./dual_currency_utils";
...
patch(OpeningControlPopup.prototype, {
    ...
    _initDualState() {
        try {
            ...
            const cfg = pos ? pos.config : null;
            if (!cfg || !cfg.show_dual_currency) return;

            this.dualState.isDualEnabled   = true;
            this.dualState.secondarySymbol = getSecondarySymbol(pos);
            this.dualState.mainSymbol      = getMainSymbol(pos);
            this.dualState.rateLabel       = getRateLabel(pos);
            this.dualState.openingCashBs   = parseFloat(pos.session?.opening_cash_bs || 0);
            this.dualState.openingPmBs     = parseFloat(pos.session?.opening_pm_bs || 0);
        } catch (e) {
    ...
    get usdToBsEquiv() {
        try {
            ...
            if (usd === 0) return '';
            return `≈ ${fmt2(secondaryToMain(usd, pos))} ${this.dualState.mainSymbol}`;
        } catch (e) { return ''; }
    },
    ...
    get bsToUsdEquiv() {
        try {
            ...
            if (bs === 0) return '';
            return `≈ ${fmt2(mainToSecondary(bs, pos))} ${this.dualState.secondarySymbol}`;
        } catch (e) { return ''; }
    },
    ...
    get dualOpeningSummary() {
        try {
            ...
            const bsInUsd   = mainToSecondary(bsAmount, pos);
    ...
    async confirm() {
        ...
        if (cfg && cfg.show_dual_currency) {
            try {
                ...
                if (pos?.session) {
                    pos.session.opening_cash_bs = bsAmount;
                    pos.session.opening_cash_usd = usdAmount;
                    pos.session.opening_pm_bs = pmBsAmount;

                    if (pos.data?.call) {
                        await pos.data.call(
                            'pos.session',
                            'set_dual_opening_cash',
                            [[pos.session.id], bsAmount, usdAmount, pmBsAmount]
                        );
                    }
                }
            } catch (e) {
